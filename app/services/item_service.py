...
def bulk_add_items(db: Session, slot_id: str, entries: list[ItemBulkEntry]) -> int:
    ...
    for e in entries:
        ...
        # capacity check (missing earlier)
        if slot.current_item_count + e.quantity > slot.capacity:
            raise ValueError("capacity_exceeded")

        if slot.current_item_count + e.quantity > settings.MAX_ITEMS_PER_SLOT:
            raise ValueError("capacity_exceeded")
        ...
        # update slot count (missing earlier)
